    ...
    for i in range(n):
        ...
        while True:
            try:
                ...
                print("⚠️ Input tidak valid! Masukkan angka, contoh: 150000")
...
def input_absen():
    if not data_karyawan:
        print("⚠️ Input data karyawan terlebih dahulu!")
        return
...
def hitung_gaji_serial():
    if not data_absen:
        print("⚠️ Input data absen terlebih dahulu!")
        return
...
def hitung_gaji_parallel():
    ...
    if omp_enabled and hasattr(omp, "parallel_for"):
        ...
        _()  # panggil fungsi paralel
    else:
        # fallback ke perhitungan serial
        for i in range(len(data_karyawan)):
            k = data_karyawan[i]
            hari_masuk = int(data_absen[i]["hari_masuk"])
            total = float(k["gaji_pokok"]) * hari_masuk
    ...
    if omp_enabled:
        ...
        print(f"⚠️ omp4py tidak aktif, perhitungan dijalankan secara SERIAL dalam {elapsed:.6f} detik.")
...
def tampilkan_gaji():
    if not data_gaji:
        print("⚠️ Belum ada data gaji yang dihitung.")
        return
...
def tampilkan_grafik():
    if not serial_times or not parallel_times:
        print("⚠️ Belum ada data perbandingan waktu eksekusi!")
